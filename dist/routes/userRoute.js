...
import { createUser, deleteUser, loginUser, updateUser, } from "../controllers/userControllers.js";
const router = express.Router();
//creación de usuario
router.post("/create", async (req, res) => {
    console.log(pc.green(`Creando usuario con datos: ${pc.blue(JSON.stringify(req.body))}`));
...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        console.error(pc.yellow(`Error al intentar iniciar sesión del usuario: ${pc.red(message)}`));
...
router.post("/update", async (req, res) => {
    ...
    try {
        ...
        if (updatedUser) {
            console.log(pc.green(`Datos del usuario con ID ${pc.blue(id)} actualizados exitosamente`));
            return res.status(200).json({ message: "Datos del usuario actualizados correctamente" });
        }
        else {
            return res.status(400).json({ message: "No se pudo actualizar los datos del usuario" });
        }
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        console.error(pc.yellow(`Error al actualizar usuario ${pc.red(id)}: ${message}`));
        return res.status(500).json({ message: "Error interno al actualizar datos" });
    }
});
